Feature: Aviva application- POS Aviva Dhan Suraksha- LI=PRO- WITHOUTEkyc
  ...
  Background: Aviva application login verification
    ...
    @SmokeTest1
  Scenario: POS Aviva Dhan Suraksha- NEW LEAD CREATION
    ...
    And I Selected checkbox Proceed without e-KYC in Status tab -->About the Customer
    And I clicked on GetOTP
    And I wait for 25 sec to enter the otp
  ...
  @SmokeTest3
  Scenario: POS Aviva Dhan Suraksha- CONTINUATION- WIP
    ...
    # And Tax Purposes in Jurisdiction
    ...
    And Down to Bottom
    And Click on Next Button
    And Entered Customer Height in Health Details DGH section
    And Entered Customer Weight
    And Click on Next Button
    And Click on Next Button
    ...
    And Select Document Type
    And Selected the Doc as Income Proof
    And Upload the Document
    And Click on OTP Authentication Tab
    And Select the Check bo and Click on get otp
    And Wait for 25 Sec for entering otp
